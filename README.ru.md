...
identity C рядом с подсветкой синтаксиса, call graph всего образа (старт в
`vmMain`, drag — панорама, скролл — зум) и CFG-граф функции с полным disasm и
HEX-байтами опкодов в узлах, вкладки строк/ловушек с переходом по
кросс-ссылкам в один клик, вкладка xref (вызывающие/вызываемые), переход в
функцию даблкликом из identity C, перекрёстная подсветка при хавере между
панелями, история навигации назад/вперёд (Backspace / Alt+Right);
переименования функций сохраняются в q3asm-совместимый `.map` рядом с файлом
(все probe'ы и `emit_qvm.ps1` подхватывают их через `--names`). Menubar:
File / View / Tools (экспорт: disasm `.txt`, identity C одной или всех
функций).

`Tools ▸ Plugins…` открывает хост вне-процессных плагинов (MCP-клиент): он
находит плагины по манифестам `resq-plugin.toml` — в каталогах `plugins/`
рядом с exe или в cwd, либо в соседних чекаутах с собранным бинарником
`target/{release,debug}` (например, репозиторий `resq-mcp`), запускает их,
показывает их инструменты и вызывает их из GUI. Протокол и референс-плагин —
в репозиториях `resq-plugin-sdk` / `resq-mcp`.
