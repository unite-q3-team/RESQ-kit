...
identity C рядом с подсветкой синтаксиса, граф CFG, вкладки строк/ловушек с
переходом по кросс-ссылкам в один клик, вкладка xref (вызывающие/вызываемые),
переход в функцию даблкликом из identity C, перекрёстная подсветка при хавере
между панелями; переименования функций сохраняются в q3asm-совместимый `.map`
рядом с файлом (все probe'ы и `emit_qvm.ps1` подхватывают их через `--names`).
